...
//Authentications
export async function getDedicatedServerToken() {
  const response = await fetch('https://prod.trackmania.core.nadeo.online/v2/authentication/token/basic', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': 'Basic ' + Buffer.from(`${config.serverUsername}:${config.serverPassword}`).toString('base64')
    },
    body: JSON.stringify({ audience: 'NadeoLiveServices' })
  });

  if (!response.ok) {
    console.error(`Error fetching dedicated server token: ${response.status} ${response.statusText}`);
    console.error(await response.text());
    throw new Error('Failed to get dedicated server token');
  }
